                ...
                if k == 2:
                    ...
                    for i in event.text:
                        if i.isdigit():
                            ...
                            break  # Прерываем цикл, если найдена цифра
                    if contains_digit:
                        self.message_send(event.user_id, 'Введите название города в формате г Москва:')
                    else:
                        return event.text
        ...
        for event in self.longpoll.listen():
            if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                if event.text.lower() == 'привет':
                    ...
                    self.message_send(
                        event.user_id, f'Привет великий соискатель, {self.params["name"]}')
                    ...
                    self.message_send(
                        event.user_id, 'Неизвестная команда, напишите "привет" для запуска бота')
